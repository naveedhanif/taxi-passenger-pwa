...
export default function ChatPanel({ bookingId, guestAccessToken, customerSessionToken, selfRole, onNewMessage, hideHeader = false, maxListHeight = "13rem" }) {
  ...
  // the very first load — same "skip first read" pattern used for
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (hasLoadedOnceRef.current) {
        const newIncoming = fetched.some((m) => m.sender_role === otherRole && !knownIdsRef.current.has(m.id));
        if (newIncoming) {
          const audio = audioRef.current;
          if (audio) {
            audio.currentTime = 0;
  ...
  }, [bookingId, guestAccessToken, customerSessionToken, otherRole]);
